...
int level_coordinate(int i){
	
	if (i = 1){return 1;}		
	else{return 2*level_coordinate(i -1) + 1;}
	}
...
int main(){
	...
	int pos[N - seq_steps] = {0};
    int dir[(N - seq_steps) * 2] = {0};
	...
	for (int i=seq_steps; i<N; i++){


		int *p = pos + i - 9;		
		int *d = dir + i - 9;

		search_part(level, testarray[i], p, d, avl);
